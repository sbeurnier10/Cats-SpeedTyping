...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff = limit 
    real_word = typed_word
    for word in word_list:
        if diff_function(typed_word,word,limit) <= diff:
            if real_word == typed_word:
                diff = diff_function(typed_word,word,limit)
                real_word = word
            elif diff_function(typed_word,word,limit) < diff:
                diff = diff_function(typed_word,word,limit)
                real_word = word
    return real_word
...
def sphinx_swaps(start, goal, limit):
    ...
    5
    """
    # BEGIN PROBLEM 6 empty strings and one-letter strings big_limit edge case
    if start == goal:
        return 0
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit == -1:  # Fill in the condition
        ...
        # BEGIN
        return 1 + min(minimum_mewtations(add,goal,limit-1),minimum_mewtations(remove,goal,limit-1),minimum_mewtations(substitute,goal,limit-1))
        # END
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    lst = [sofar[i] == prompt[i] for i in range(len(sofar))]
    if all(lst):
        num_correct = len(sofar)
    else:
        num_correct = lst.index(False)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        timestamp = times_per_player[i]
        time_diff = [timestamp[j + 1] - timestamp[j] for j in range(len(timestamp) - 1)] 
...
def fastest_words(match):
    ...
    for j in word_indices:
        lst = [times[i][j] for i in player_indices]
        fastest = lst.index(min(lst))
        lst_fastest[fastest].append(words[j])
    return lst_fastest
    # END PROBLEM 10
